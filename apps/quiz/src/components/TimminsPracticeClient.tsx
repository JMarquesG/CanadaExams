...
import { timminsQuestions, TimminsQuestion } from "@/data/timmins-questions";
...
export default function TimminsPracticeClient({ sessionId: resumeSessionId, weakIds, count }: { sessionId?: string; weakIds?: number[]; count?: number }) {
  const [quiz, setQuiz] = useState<TimminsQuestion[]>([]);
  ...
  useEffect(() => {
    ...
    let pool = timminsQuestions;
    if (weakIds && weakIds.length > 0) {
      pool = weakIds.map((id) => timminsQuestions.find((q) => q.id === id)).filter(Boolean) as TimminsQuestion[];
    }
  ...
  useEffect(() => {
    if (quiz.length > 0) {
      recordTimminsQuestionView(quiz[0].id);
    }
  }, [quiz]);
  ...
  const handleRestart = () => {
    const shuffled = shuffleArray(timminsQuestions);
    setQuiz(shuffled);
    setCurrentIndex(0);
    setAnswers({});
  ...
  const QuestionGrid = ({
    ...
    <div className="flex flex-wrap gap-1.5">
      {quiz.map((_, idx) => {
        ...
        let bg: string;
        if (isCurrent) {
          bg = "bg-teal-700 text-white ring-2 ring-teal-300";
        } else if (a) {
          bg = a.correct
  ...
  if (done) {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <header className="bg-teal-800 text-white py-6 px-4">
          <div className="max-w-3xl mx-auto flex items-center gap-4">
            <Link href="/?bank=timmins" className="text-teal-300 hover:text-white text-sm">← Home</Link>
            <h1 className="text-xl font-semibold">Timmins Practice — Report</h1>
          </div>
        </header>

        <main className="max-w-3xl mx-auto px-4 py-10">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center mb-8">
            <div className={`text-6xl font-bold mb-2 ${pct >= 70 ? "text-emerald-600" : "text-amber-500"}`}>{pct}%</div>
            ...
            <div className="flex gap-4 justify-center text-sm mb-6">
              ...
              {total < quiz.length && <span className="text-gray-400">— {quiz.length - total} skipped</span>}
            </div>
            <div className="flex gap-3 justify-center">
              <button onClick={handleRestart} className="bg-teal-700 hover:bg-teal-800 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors">Practice Again</button>
          ...
          {reviewQ && reviewA && (
            <div className={`bg-white rounded-lg border-2 p-6 mb-6 ${reviewA.correct ? "border-green-300" : "border-red-300"}`}>
              ...
              <div className="flex justify-between mt-4">
                <button
                  onClick={() => { const arr = [...filteredSet].sort((a, b) => a - b); const pos = arr.indexOf(reviewIndex!); if (pos > 0) setReviewIndex(arr[pos - 1]); }}
                ...
                >← Previous</button>
                <button
                  onClick={() => { const arr = [...filteredSet].sort((a, b) => a - b); const pos = arr.indexOf(reviewIndex!); if (pos < arr.length - 1) setReviewIndex(arr[pos + 1]); }}
          ...
          {reviewIndex !== null && !reviewA && (
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
              ...
              <p className="text-gray-800 font-medium text-sm leading-relaxed whitespace-pre-line mb-4">{quiz[reviewIndex].question}</p>
              <p className="text-xs text-green-700">
                Correct answer: ({quiz[reviewIndex].correctAnswer}) {quiz[reviewIndex].options.find((o) => o.id === quiz[reviewIndex].correctAnswer)?.text}
              </p>
            </div>
          )}
            ...
            <>
              ...
              <div className="space-y-3">
                {quiz.map((q, idx) => {
                  ...
                  if (!a) return null;
                  return (
                    <button key={q.id} onClick={() => setReviewIndex(idx)}
                      className={`w-full text-left bg-white rounded-lg border p-4 hover:shadow-sm transition-shadow ${a.correct ? "border-green-200" : "border-red-200"}`}>
                      <div className="flex items-start gap-2 mb-1">
                        <span className={`text-sm font-medium shrink-0 ${a.correct ? "text-green-600" : "text-red-600"}`}>{a.correct ? "✓" : "✗"} Q{idx + 1}</span>
                        <p className="text-sm text-gray-700 flex-1">{q.question.split("\n")[0]}</p>
                      </div>
                      {!a.correct && <p className="text-xs text-red-500 ml-6">Your answer: {q.options.find((o) => o.id === a.selected)?.text}</p>}
                    </button>
                  );
                })}
              </div>
  ...
  const handleSelect = (optionId: number) => {
    ...
    setAnswers((prev) => ({ ...prev, [currentIndex]: { selected: optionId, correct } }));
    recordTimminsQuestionAnswer(current.id, correct);
  ...
  const handleNext = () => {
    if (currentIndex < quiz.length - 1) {
      ...
      if (sessionIdRef.current) {
        finishTimminsSession(sessionIdRef.current, total, correctCount);
      }
      setDone(true);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-teal-800 text-white py-4 px-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/?bank=timmins" className="text-teal-300 hover:text-white text-sm">← Home</Link>
            <span className="text-teal-500">|</span>
            <span className="text-sm text-teal-200 font-medium">{weakIds && weakIds.length > 0 ? "🎯 Weakest Questions" : "⚡ Timmins Practice"}</span>
          </div>
      ...
      <div className="bg-teal-900 h-1">
        <div className="bg-teal-400 h-1 transition-all duration-300" style={{ width: `${((currentIndex + 1) / quiz.length) * 100}%` }} />
      </div>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="space-y-3 mb-6">
          {current.options.map((option) => {
            ...
            return (
              <button key={option.id} onClick={() => handleSelect(option.id)} disabled={isRevealed}
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {!isCorrect && (
                <span className="text-sm text-gray-500">
                  — Correct answer: <strong className="text-gray-700">({current.correctAnswer}) {current.options.find((o) => o.id === current.correctAnswer)?.text}</strong>
                </span>
              )}
            </div>
            {current.explanation && <p className="text-sm text-gray-600 mt-2">{current.explanation}</p>}
          </div>
        )}
